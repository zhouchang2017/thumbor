...
// 让ImageSpec 可以通过字符串创建；s.parse().unwarp()
...
impl From<resize::SampleFilter> for SamplingFilter {
    fn from(f: resize::SampleFilter) -> Self {
        match f {
...
impl Spec {
    ...
    pub fn new_filter(filter: filter::Filter) -> Self {
        Self {
            data: Some(spec::Data::Fliter(Filter {
                filter: filter as i32,
            })),
        }
    }
...
mod tests {
    ...
    #[test]
    fn encode_spec_colud_be_decode() {
        Spec::new_filter(filter::Filter::Marine);
        let spec = ImageSpec::new(vec![
        ...
        println!("{}",s);
        assert_eq!(spec,s.as_str().try_into().unwrap());
    }
}
